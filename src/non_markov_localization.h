...
class NonMarkovLocalization {
 public:
  ...
  // Typedef for timestamps. First entry is the seconds, second entry is
  // nanoseconds.
  ...
  std::vector<Timestamp> GetLoggedStamps() const;
 ...
 private:
  ...
  std::vector<Timestamp> pending_stamps_;
  ...
  // saved by the TrimEpisode function when the poses are removed from the
  // latest non-Markov episode.
  ThreadSafe<std::vector<Timestamp>> logged_stamps_;
