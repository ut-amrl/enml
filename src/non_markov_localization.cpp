   ...
   public:
    SRLCallback(const vector<double>& poses,
                ...
                void (*callback)(
                    ...
                    const PointCloudf& point_cloud_e,
                    const NormalCloudf& normal_cloud,
                    const vector<LTSConstraint*>& constraints)) : poses_(poses),
                                                                  point_cloud_e_(point_cloud_e),
...
NonMarkovLocalization::NonMarkovLocalization(const string& maps_directory) : lost_metric_(0),
                                                                             terminate_(false),
                                                                             next_pose_id_(0),
                                                                             t_last_update_(0),
        ...
        for (size_t j = 0; j < point_cloud.size(); ++j) {
            ...
            if (fabs(line_normal.dot(normal)) > cos(DegToRad(40.0)) &&
                (line_normal.dot(pose_location) + line_offset) *
                        (line_normal.dot(point_global) + line_offset) <
                    -FLT_MIN &&
                ...
                if (kUseRelativeConstraints) {
                    ...
                    DynamicAutoDiffCostFunction<VisibilityRelativeConstraint,
                                                kDynamicDiffStride>* cost_function = new DynamicAutoDiffCostFunction<VisibilityRelativeConstraint,
                                                                                                                     kDynamicDiffStride>(constraint);
                    vector<double*> parameter_blocks;
