...
amrl_msgs::msg::VisualizationMsg visualization_msg_;
...
// ROS2 publishers are now managed by the EnmlNode class
...
// Global ROS2 node for compatibility with existing callback functions
std::shared_ptr<rclcpp::Node> global_node = nullptr;

// Forward declarations for callback functions
void LaserCallback(const sensor_msgs::msg::LaserScan& laser_message);
void OdometryCallback(const nav_msgs::msg::Odometry& msg);
void InitializeCallback(const amrl_msgs::msg::Localization2DMsg& msg);
geometry_msgs::msg::PoseStamped ConvertAMRLmsgToROSmsg(const amrl_msgs::msg::Localization2DMsg& pose_amrl);
...
class EnmlNode : public rclcpp::Node {
   public:
    ...
    void setupSubscribers() {
        // Setup subscribers for online mode
        laser_subscriber_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
            CONFIG_scan_topic, rclcpp::QoS(1),
            std::bind(&EnmlNode::laserCallback, this, std::placeholders::_1));

        odom_subscriber_ = this->create_subscription<nav_msgs::msg::Odometry>(
            CONFIG_odom_topic, rclcpp::QoS(1),
    ...
    void publishLocation(const string& map_name, const float x, const float y, const float angle) {
        ...
        localization_msg_.header.stamp = this->now();
   ...
   private:
    ...
    void initializeCallback(const amrl_msgs::msg::Localization2DMsg::SharedPtr msg) {
        if (localization_ != nullptr) {
    ...
    rclcpp::Publisher<amrl_msgs::msg::Localization2DMsg>::SharedPtr localization_publisher_amrl_;
    rclcpp::Publisher<geometry_msgs::msg::PoseStamped>::SharedPtr localization_publisher_ros_;

    rclcpp::Subscription<sensor_msgs::msg::LaserScan>::SharedPtr laser_subscriber_;
    rclcpp::Subscription<nav_msgs::msg::Odometry>::SharedPtr odom_subscriber_;
    rclcpp::Subscription<amrl_msgs::msg::Localization2DMsg>::SharedPtr initialize_subscriber_;
};

// Update the global variables to use the ROS2 node
std::shared_ptr<EnmlNode> enml_node = nullptr;
...
void ApplyNoiseModel(
    ...
    M_enc_to_vel << kSqRt2, -kSqRt2, -kSqRt2, kSqRt2,
...
void PublishLocation(
    ...
    if (enml_node) {
        enml_node->publishLocation(map_name, x, y, angle);
    }
}
...
void PublishDisplay() {
    if (enml_node) {
        enml_node->publishVisualization();
    }
}
...
#define ENML_STRING_CONFIG(x) \
    ...
    config_reader::ConfigReader reader({std::string(config_dir_) + "/common.lua",
...
bool AddPose(const std::shared_ptr<sensor_msgs::msg::LaserScan>& laser_message,
    ...
    const double timestamp = laser_message->header.stamp.sec + laser_message->header.stamp.nanosec / 1e9;
    const bool keyframe = binary_search(keyframes.begin(), keyframes.end(), timestamp);
    if (!keyframe && relative_location.norm() < localization_options_.minimum_node_translation &&
...
bool LoadLaserMessage(const std::shared_ptr<rosbag2_storage::SerializedBagMessage>& serialized_msg,
    ...
    if (topic_name == CONFIG_scan_topic) {
        ...
        auto laser_message = std::make_shared<sensor_msgs::msg::LaserScan>();
        ...
        serialization.deserialize_message(&serialized_data, laser_message.get());
...
bool LoadOdometryMessage(const std::shared_ptr<rosbag2_storage::SerializedBagMessage>& serialized_msg,
                         const std::string& topic_name,
                         const rclcpp::Time& message_time,
    ...
    if (topic_name == CONFIG_odom_topic) {
        ...
        rclcpp::SerializedMessage serialized_data(*serialized_msg->serialized_data);
        serialization.deserialize_message(&serialized_data, odometry_message.get());
...
bool LoadSetLocationMessage(const std::shared_ptr<rosbag2_storage::SerializedBagMessage>& serialized_msg,
                            const std::string& topic_name,
                            const rclcpp::Time& message_time,
                            Vector2f* global_location,
                            float* global_angle,
    ...
    if (topic_name == CONFIG_initialpose_topic) {
        // Deserialize the message
        auto set_location_message = std::make_shared<amrl_msgs::msg::Localization2DMsg>();
        rclcpp::Serialization<amrl_msgs::msg::Localization2DMsg> serialization;
        rclcpp::SerializedMessage serialized_data(*serialized_msg->serialized_data);
        serialization.deserialize_message(&serialized_data, set_location_message.get());
...
void LoadRosBag(const string& bagName, int max_laser_poses, double time_skip,
    ...
    try {
        ...
        printf("Error opening bag file: %s\n", e.what());
        return;
    }
    ...
    const std::vector<std::string> topics({CONFIG_scan_topic,
                                           CONFIG_odom_topic,
    ...
    while (reader.has_next() && run_ &&
        ...
        if (LoadSetLocationMessage(serialized_message, serialized_message->topic_name,
                                   message_time, &global_location,
                                   &global_angle, map_name)) {
            continue;
        }

        // Check to see if this is a laser scan message.
        if (LoadLaserMessage(serialized_message, serialized_message->topic_name,
                             message_time, keyframes, &relative_location,
        ...
        if (LoadOdometryMessage(serialized_message, serialized_message->topic_name,
                                message_time, odometry_location, odometry_angle,
...
void StandardOdometryCallback(const nav_msgs::msg::Odometry& last_odometry_msg,
    ...
    if (debug_level_ > 1) {
        printf("Standard Odometry %8.3f %8.3f %8.3f, t=%f\n",
               ...
               odometry_msg.header.stamp.sec + odometry_msg.header.stamp.nanosec / 1e9);
    }
    if (test_set_index_ >= 0 || statistical_test_index_ >= 0) {
...
void OdometryCallback(const nav_msgs::msg::Odometry& msg) {
    static nav_msgs::msg::Odometry last_msg_;
...
void LaserCallback(const sensor_msgs::msg::LaserScan& laser_message) {
    ...
    if (debug_level_ > 1) {
        printf("LaserScan, t=%f\n", laser_message.header.stamp.sec + laser_message.header.stamp.nanosec / 1e9);
    }
    PointCloudf point_cloud;
    const Vector2f sensor_offset(
        localization_options_.sensor_offset.x(),
...
float CheckLaserOverlap(const sensor_msgs::msg::LaserScan& scan1,
                        const Pose2Df& pose1,
    ...
    for (size_t i = 0; i < ranges1.size(); ++i) {
        ...
        for (; j < new_point_cloud.size() &&
               new_point_cloud[j].first < scan_angle;
             ++j) {
        }
...
void SensorResettingResample(const sensor_msgs::msg::LaserScan& laser_message) {
...
void PlayBagFile(const string& bag_file,
    ...
    nav_msgs::msg::Odometry last_standard_odometry;
    sensor_msgs::msg::LaserScan last_laser_scan;
    ...
    while (reader.has_next() && run_) {
        ...
        if (!quiet_ && debug_level_ > 1) {
            printf("message from %s\n", serialized_message->topic_name.c_str());
        }
        // Ignore messages before elapsed time_skip.
        if (bag_time < bag_time_start + time_skip) continue;
        if (!quiet_) {
        ...
        if (run_ && kbhit() != 0) {
            ...
            switch (keycode) {
                case ' ': {
                    printf("Paused\n");
                    keycode = 0;
                    while (keycode != ' ' && run_ && rclcpp::ok()) {
                        Sleep(0.01);
                        if (enml_node != nullptr) {
                            rclcpp::spin_some(enml_node);
                        }
                        if (kbhit() != 0) {
                            keycode = fgetc(stdin);
                        }
        ...
        // Check to see if this is a laser scan message.
        if (serialized_message->topic_name == CONFIG_scan_topic) {
            // Deserialize the message
            auto laser_message = std::make_shared<sensor_msgs::msg::LaserScan>();
            rclcpp::Serialization<sensor_msgs::msg::LaserScan> serialization;
        ...
        if (serialized_message->topic_name == CONFIG_odom_topic) {
            // Deserialize the message
            auto odometry_message = std::make_shared<nav_msgs::msg::Odometry>();
            rclcpp::Serialization<nav_msgs::msg::Odometry> serialization;
            rclcpp::SerializedMessage serialized_data(*serialized_message->serialized_data);
...

        {
            ...
            string init_map;
            if (LoadSetLocationMessage(serialized_message, serialized_message->topic_name,
                                       message_time, &init_location, &init_angle,
                                       &init_map)) {
                if (debug_level_ > 0) {
...
void InitializeCallback(const amrl_msgs::msg::Localization2DMsg& msg) {
    ...
    if (enml_node) {
        enml_node->publishLocation(msg.map, msg.pose.x, msg.pose.y, msg.pose.theta);
    }
...
int main(int argc, char** argv) {
    ...
    bool running_tests = false;  // Add this line
    char* bag_file = NULL;
    ...
    if (bag_file != NULL) {
        PlayBagFile(
            bag_file, max_laser_poses, !disable_stfs, time_skip, NULL, keyframes_file, rate);
    } else {
        OnlineLocalize(!disable_stfs);
    }
