...
void ApplyNoiseModel(const float dx, const float dy, const float da, const float e, float* dx_n, float* dy_n, float* da_n) {
  // Wheel radius.
  const float R = 0.1;
  Eigen::Matrix<float, 4, 3> M_vel_to_enc;
  Eigen::Matrix<float, 3, 4> M_enc_to_vel;
  const float C = cos(DegToRad(45.0));
  M_vel_to_enc << C, C, R, -C, C, R, -C, -C, R, C, -C, R;
  const float kSqRt2 = sqrt(2.0);
  M_enc_to_vel << kSqRt2, -kSqRt2, -kSqRt2, kSqRt2, kSqRt2, kSqRt2, -kSqRt2, -kSqRt2, 1.0 / R, 1.0 / R, 1.0 / R, 1.0 / R;
...
void PublishLocation(const string& map_name, const float x, const float y, const float angle) {
...
void PublishTrace() {
  ...
  const Vector2f curLoc(latest_pose.translation.x(), latest_pose.translation.y());
  if (!initialized) {
...
void ClearDisplay() { visualization::ClearVisualizationMsg(visualization_msg_); }

void PublishDisplay() { visualization_publisher_.publish(visualization_msg_); }
...
#define ENML_FLOAT_CONFIG(x) CONFIG_FLOAT(x, "enml." #x)
#define ENML_INT_CONFIG(x) CONFIG_INT(x, "enml." #x)
#define ENML_UINT_CONFIG(x) CONFIG_UINT(x, "enml." #x)
#define ENML_BOOL_CONFIG(x) CONFIG_BOOL(x, "enml." #x)
#define ENML_STRING_CONFIG(x) CONFIG_STRING(x, "enml." #x)
  ...
  config_reader::ConfigReader reader({std::string(config_dir_) + "/common.lua", std::string(config_dir_) + "/" + std::string(robot_config_)});
...
void OutputPosesAndStamps(const std::string& poses_file, const std::vector<Pose2Df>& poses, const std::vector<timespec>& stamps) {
  ...
  for (size_t i = 0; i < poses.size(); i++) {
    ...
    csv_file << stamp.tv_sec << ", " << stamp.tv_nsec << ", " << pose.translation.x() << ", " << pose.translation.y() << ", " << pose.angle << "\n";
  }

  csv_file.close();
}

void SaveStfs(const string& map_name, const vector<Pose2Df>& poses, const vector<PointCloudf>& point_clouds, const vector<NormalCloudf>& normal_clouds, const vector<vector<NonMarkovLocalization::ObservationType> >& classifications, const string& bag_file, double timestamp) {
  ...
  for (size_t i = 0; i < point_clouds.size(); ++i) {
    ...
    for (unsigned int j = 0; j < point_clouds[i].size(); ++j) {
      ...
      if (!kSaveAllObservations && classifications[i][j] != NonMarkovLocalization::kStfObservation) {
        if (kDisplaySteps) {
      ...
      fprintf(fid(), "%.4f,%.4f,%.4f, %.4f,%.4f, %.4f,%.4f\n", poses[i].translation.x(), poses[i].translation.y(), poses[i].angle, p.x(), p.y(), n.x(), n.y());
      if (kDisplaySteps) {
        visualization::DrawPoint(p, kStfPointColor, visualization_msg_);
      }
    }
    if (kDisplaySteps) {
      PublishLocation(kMapName, poses[i].translation.x(), poses[i].translation.y(), poses[i].angle);
      PublishDisplay();
      while (kbhit() == 0) {
        Sleep(0.02);
      }
...
void SaveEpisodeStats(FILE* fid) {
  ...
  Pose2Df latest_pose;
  if (!localization_->GetNodeData(&poses, &pose_ids, &point_clouds, &pending_poses, &observation_classes, &latest_pose) || poses.size() < 1 || poses.size() != observation_classes.size()) {
    return;
  }
  ...
  fprintf(fid, "%s, %f, %f, %f, %f, %f, %f, %d, %d, %d, %d\n", map_name.c_str(), poses.back().translation.x(), poses.back().translation.y(), poses.back().angle, poses.front().translation.x(), poses.front().translation.y(), poses.front().angle, episode_length, num_ltfs, num_stfs, num_dfs);
}

bool AddPose(const sensor_msgs::LaserScanPtr& laser_message, const vector<double>& keyframes, Vector2f* relative_location_ptr, float* relative_angle_ptr, Vector2f* global_location_ptr, float* global_angle_ptr, vector<PointCloudf>* point_clouds, vector<NormalCloudf>* normal_clouds, vector<Pose2Df>* poses, vector<double>* timestamps, Vector2f* odometry_location, float* odometry_angle) {
  ...
  const bool keyframe = binary_search(keyframes.begin(), keyframes.end(), laser_message->header.stamp.toSec());
  if (!keyframe && relative_location.norm() < localization_options_.minimum_node_translation && fabs(relative_angle) < localization_options_.minimum_node_rotation) {
    // Ignore this pose since the robot has not moved since the last update.
    return false;
  }
  if (debug) {
    printf("Relative pose: %6.3f, %6.3f %4.1f\u00b0\n", relative_location.x(), relative_location.y(), DegToRad(relative_angle));
  }
  // Update global pose
  global_location = Rotation2Df(global_angle) * relative_location + global_location;
  global_angle = relative_angle + global_angle;

  // Update odometry pose
  *odometry_location = Rotation2Df(*odometry_angle) * relative_location + *odometry_location;
  *odometry_angle = relative_angle + *odometry_angle;
  ...
  const Vector2f sensor_offset(localization_options_.sensor_offset.x(), localization_options_.sensor_offset.y());
  for (unsigned int i = 0; i < laser_message->ranges.size(); ++i) {
    if (laser_message->ranges[i] < kMinPointCloudRange || laser_message->ranges[i] > kMaxPointCloudRange || !std::isfinite(laser_message->ranges[i])) continue;
    const float angle = laser_message->angle_min + laser_message->angle_increment * static_cast<float>(i);
    if (angle < laser_message->angle_min + DegToRad(15.0) || angle > laser_message->angle_max - DegToRad(15.0)) continue;
    float range = laser_message->ranges[i];
    if (use_laser_corrections_) {
      const int angle_index = floor((angle + M_PI) / laser_corrections_resolution_);
    ...
    point_cloud.push_back(sensor_offset + Rotation2Df(angle) * Vector2f(range, 0.0));
  }
...
bool LoadLaserMessage(const rosbag::MessageInstance& message, const vector<double>& keyframes, Vector2f* relative_location, float* relative_angle, Vector2f* global_location, float* global_angle, vector<PointCloudf>* point_clouds, vector<NormalCloudf>* normal_clouds, vector<Pose2Df>* poses, vector<double>* timestamps, Vector2f* odometry_location, float* odometry_angle) {
  sensor_msgs::LaserScanPtr laser_message = message.instantiate<sensor_msgs::LaserScan>();
  if (laser_message != NULL && message.getTopic() == CONFIG_scan_topic) {
    if (false && debug_level_ > 1) {
      printf("Laser Msg,    t:%.2f\n", message.getTime().toSec());
      fflush(stdout);
    }
    AddPose(laser_message, keyframes, relative_location, relative_angle, global_location, global_angle, point_clouds, normal_clouds, poses, timestamps, odometry_location, odometry_angle);
    return true;
  }
  return false;
}

bool LoadOdometryMessage(const rosbag::MessageInstance& message, const Vector2f& odometry_location, const float& odometry_angle, Vector2f* relative_location, float* relative_angle) {
  nav_msgs::OdometryPtr odometry_message = message.instantiate<nav_msgs::Odometry>();
  ...
  if (odometry_message != NULL && message.getTopic() == CONFIG_odom_topic) {
    ...
    const Vector2f odometry_message_location(odometry_message->pose.pose.position.x, odometry_message->pose.pose.position.y);
    *relative_location = kOdometryTranslationScale * (Rotation2Df(-odometry_angle) * (odometry_message_location - odometry_location));
    const float odometry_message_angle = 2.0 * atan2(odometry_message->pose.pose.orientation.z, odometry_message->pose.pose.orientation.w);
    *relative_angle = kOdometryRotationScale * AngleDiff(odometry_message_angle, odometry_angle);
    if (test_set_index_ >= 0 || statistical_test_index_ >= 0) {
      relative_location->x() += rand_.Gaussian(0, odometry_additive_noise_ * relative_location->x());
      relative_location->y() += rand_.Gaussian(0, odometry_additive_noise_ * relative_location->y());
      (*relative_angle) += rand_.Gaussian(0, odometry_additive_noise_ * (*relative_angle));
    }
    return true;
  }
...
bool LoadSetLocationMessage(const rosbag::MessageInstance& message, Vector2f* global_location, float* global_angle, string* map_name) {
  amrl_msgs::Localization2DMsgPtr set_location_message = message.instantiate<amrl_msgs::Localization2DMsg>();
  const string topic_name = message.getTopic();
  if (set_location_message != NULL && message.getTopic() == CONFIG_initialpose_topic) {
    if (debug_level_ > 1) {
...
void LoadRosBag(const string& bagName, int max_laser_poses, double time_skip, const vector<double>& keyframes, vector<PointCloudf>* point_clouds, vector<NormalCloudf>* normal_clouds, vector<Pose2Df>* poses, vector<double>* timestamps, double* t_duration, string* map_name) {
  ...
  rosbag::Bag bag;
  printf("Opening bag file %s...", bagName.c_str());
  fflush(stdout);
  bag.open(bagName, rosbag::bagmode::Read);
  printf(" Done.\n");
  fflush(stdout);

  const std::vector<std::string> topics({CONFIG_scan_topic, CONFIG_odom_topic, CONFIG_initialpose_topic});
  ...
  double bag_time = 0.0;
  for (rosbag::View::iterator it = view.begin(); run_ && it != view.end() && (max_laser_poses < 0 || static_cast<int>(poses->size()) < max_laser_poses); ++it) {
    ...
    if (LoadSetLocationMessage(message, &global_location, &global_angle, map_name)) {
      continue;
    }

    // Check to see if this is a laser scan message.
    if (LoadLaserMessage(message, keyframes, &relative_location, &relative_angle, &global_location, &global_angle, point_clouds, normal_clouds, poses, timestamps, &odometry_location, &odometry_angle)) {
      continue;
    }

    // Check to see if this is an odometry message.
    if (LoadOdometryMessage(message, odometry_location, odometry_angle, &relative_location, &relative_angle)) {
      continue;
    }
  }
...
void DrawVisibilityConstraints(const vector<vector_localization::VisibilityGlobConstraint*>& visibility_constraints, const vector<double>& poses) {
  return;
  // TODO(Joydeepb): This causes a segfault when "enml.limit_history = true;".
  for (size_t i = 0; i < visibility_constraints.size(); ++i) {
    const vector_localization::VisibilityGlobConstraint& constraint = *visibility_constraints[i];
    const size_t& pose_index = constraint.pose_index;
    const Vector2f pose_translation(poses[3 * pose_index], poses[3 * pose_index + 1]);
    const Affine2f pose_transform = Translation2f(poses[3 * pose_index], poses[3 * pose_index + 1]) * Rotation2Df(poses[3 * pose_index + 2]);
    for (size_t j = 0; j < constraint.points.size(); ++j) {
      ...
      const float point_offset = constraint.line_normals[j].dot(point_global - constraint.line_p1s[j]);
      const Vector2f point_anchor = point_global - constraint.line_normals[j] * point_offset;
      ...
      const uint32_t colour = (static_cast<uint32_t>(255.0 * alpha) << 24) | 0xFF0000;
      visualization::DrawLine(point_global, pose_translation, colour, visualization_msg_);
    }
  }
}

void DrawStfs(const vector<NonMarkovLocalization::PointToPointGlobCorrespondence>& point_point_correspondences, const vector<double>& poses, const vector<vector<Vector2f> >& point_clouds, const std::vector<NormalCloudf>& normal_clouds) {
  ...
  for (size_t i = 0; i < point_point_correspondences.size(); ++i) {
    ...
    CHECK_EQ(point_point_correspondences[i].points0.size(), point_point_correspondences[i].points1.size());
    const Affine2f pose_tf0 = Translation2f(poses[3 * pose_index0 + 0], poses[3 * pose_index0 + 1]) * Rotation2Df(poses[3 * pose_index0 + 2]);
    const Affine2f pose_tf1 = Translation2f(poses[3 * pose_index1 + 0], poses[3 * pose_index1 + 1]) * Rotation2Df(poses[3 * pose_index1 + 2]);
...
void DrawLtfs(const size_t start_pose, const size_t end_pose, const vector<double>& poses, const vector<vector<Vector2f> >& point_clouds, const vector<vector<NonMarkovLocalization::ObservationType> >& classifications, const vector<vector<Line2f> >& ray_cast_lines, const vector<vector<int> >& point_line_correspondences) {
  ...
  if (kDrawPredictedScan) {
    ...
    for (size_t i = start_pose; i <= end_pose; ++i) {
      ...
      const Vector2f laser_loc = pose_location + pose_rotation * sensor_offset;
      static const int kNumRays = 180;
      static const float kAngleMax = DegToRad(135);
      vector<float> scan;
      map_.GetPredictedScan(laser_loc, 0, localization_options_.kMaxRange, pose_angle - kAngleMax, pose_angle + kAngleMax, kNumRays, &scan);
  ...
  for (size_t i = start_pose; i <= end_pose; ++i) {
    ...
    for (size_t j = 0; j < point_clouds[i].size(); ++j) {
      ...
      visualization::DrawLine(point, point_projected, kLtfCorrespondenceColor, visualization_msg_);
    }
  }
}
...
void DrawGradients(const size_t start_pose, const size_t end_pose, const vector<double>& gradients, const vector<double>& poses) {
  if (gradients.size() != poses.size()) return;
  for (size_t i = start_pose, j = 3 * start_pose; i <= end_pose; ++i, j += 3) {
...
void DrawPoseCovariance(const Vector2f& pose, const Matrix2f& covariance) {
  ...
  for (float a = 0; a < 2.0 * M_PI; a += kDTheta) {
    const Vector2f v1(cos(a) * sqrt(eigenvalues(0)), sin(a) * sqrt(eigenvalues(1)));
    const Vector2f v2(cos(a + kDTheta) * sqrt(eigenvalues(0)), sin(a + kDTheta) * sqrt(eigenvalues(1)));
...
void DrawPoses(const size_t start_pose, const size_t end_pose, const vector<Pose2Df>& odometry_poses, const vector<double>& poses, const vector<Matrix2f>& covariances) {
  ...
  for (size_t i = start_pose; i <= end_pose; ++i) {
    ...
    if (i > start_pose) {
      ...
      const Vector2f odometry = Rotation2Df(pose_angle_last) * Rotation2Df(-odometry_poses[i - 1].angle) * (odometry_poses[i].translation - odometry_poses[i - 1].translation);
...
void DrawRayCasts(const size_t start_pose, const size_t end_pose, const vector<vector<Line2f> >& ray_cast_lines, const vector<double>& poses) {
  const uint32_t colour = 0xFFCF8D00;
  for (size_t i = start_pose; i <= end_pose; ++i) {
    const Vector2f pose_location(poses[3 * i + 0], poses[3 * i + 1]);
    for (size_t j = 0; j < ray_cast_lines[i].size(); ++j) {
      visualization::DrawLine(ray_cast_lines[i][j].p0, ray_cast_lines[i][j].p1, colour, visualization_msg_);
      /*
...
void DrawFactor(const Vector2f& p0, const Vector2f& p1, const uint32_t edge_color, const uint32_t factor_node_color, const uint32_t variable_node_color) {
...
void DrawFactorGraph(const vector<double>& poses, const vector<vector<int> >& point_line_correspondences, const vector<NonMarkovLocalization::PointToPointGlobCorrespondence>& point_point_correspondences, const size_t start_pose, const size_t end_pose) {
  ...
  for (size_t i = start_pose; i < end_pose; ++i) {
    ...
    DrawFactor(p0, p1, kFactorEdgeColor, kFactorFactorNodeColor, kFactorVariableNodeColor);
  }
...
void CorrespondenceCallback(const vector<double>& poses, const vector<vector<Vector2f> >& point_clouds, const vector<NormalCloudf>& normal_clouds, const vector<vector<Line2f> >& ray_cast_lines, const vector<vector<int> >& point_line_correspondences, const vector<NonMarkovLocalization::PointToPointGlobCorrespondence>& point_point_correspondences,
                            const vector<vector<NonMarkovLocalization::ObservationType> >& classifications, const vector<vector_localization::VisibilityGlobConstraint*>& visibility_constraints, const vector<double>& gradients, const vector<Matrix2f>& covariances, const vector<Pose2Df>& odometry_poses, const size_t start_pose, const size_t end_pose) {
  ...
  DrawObservations(start_pose, end_pose, poses, point_clouds, normal_clouds, classifications);
  ...
  if (kDisplayFactorGraph) {
    DrawFactorGraph(poses, point_line_correspondences, point_point_correspondences, start_pose, end_pose);
  }
  if (kDisplayLtfCorrespondences) {
    DrawLtfs(start_pose, end_pose, poses, point_clouds, classifications, ray_cast_lines, point_line_correspondences);
  }
...
void SaveLoggedPoses(const string& filename, const vector<Pose2Df>& logged_poses, const vector<double>& timestamps) {
  ScopedFile fid(filename, "w");
  for (size_t i = 0; i < logged_poses.size(); ++i) {
    fprintf(fid(), "%f %f %f %f\n", timestamps[i], logged_poses[i].translation.x(), logged_poses[i].translation.y(), logged_poses[i].angle);
  }
}
...
void SaveSensorErrors(const vector<Pose2Df>& poses, const vector<PointCloudf>& point_clouds, const vector<vector<NonMarkovLocalization::ObservationType> >& classifications) {
  VectorMap map(maps_dir_ + "/" + kMapName + ".txt");
  ScopedFile fid("results/sensor_errors.txt", "w");
  printf("Saving sensor errors... ");
  fflush(stdout);
  visualization::ClearVisualizationMsg(visualization_msg_);
  const Vector2f laser_loc(localization_options_.sensor_offset.x(), localization_options_.sensor_offset.y());
  for (size_t i = 0; i < poses.size(); ++i) {
    const PointCloudf& point_cloud = point_clouds[i];
    const Vector2f pose_loc_g = poses[i].translation + Rotation2Df(poses[i].angle) * laser_loc;
    ...
    for (size_t j = 0; j < point_cloud_g.size(); ++j) {
      ...
      if (classifications[i][j] != NonMarkovLocalization::kLtfObservation || not_expected) {
        continue;
      }
...
void StandardOdometryCallback(const nav_msgs::Odometry& last_odometry_msg, const nav_msgs::Odometry& odometry_msg) {
  const float old_theta = 2.0 * atan2(last_odometry_msg.pose.pose.orientation.z, last_odometry_msg.pose.pose.orientation.w);
  const float new_theta = 2.0 * atan2(odometry_msg.pose.pose.orientation.z, odometry_msg.pose.pose.orientation.w);
  const Vector2f p_old(last_odometry_msg.pose.pose.position.x, last_odometry_msg.pose.pose.position.y);
  const Vector2f p_new(odometry_msg.pose.pose.position.x, odometry_msg.pose.pose.position.y);
  Vector2f p_delta = Rotation2Df(-old_theta) * (p_new - p_old);
  float d_theta = AngleDiff(new_theta, old_theta);
  if (debug_level_ > 1) {
    printf("Standard Odometry %8.3f %8.3f %8.3f, t=%f\n", p_delta.x(), p_delta.y(), RadToDeg(d_theta), odometry_msg.header.stamp.toSec());
  }
  if (test_set_index_ >= 0 || statistical_test_index_ >= 0) {
    ApplyNoiseModel(p_delta.x(), p_delta.y(), d_theta, odometry_additive_noise_, &(p_delta.x()), &(p_delta.y()), &(d_theta));
  }
  localization_->OdometryUpdate(kOdometryTranslationScale * p_delta.x(), kOdometryTranslationScale * p_delta.y(), d_theta);
  PublishLocation();
}
...
void LaserCallback(const sensor_msgs::LaserScan& laser_message) {
  ...
  NormalCloudf normal_cloud;
  GenerateNormals(kMaxNormalPointDistance, normal_weights_, &point_cloud, &normal_cloud);
...
bool LaserPointComparator(const pair<float, Vector2f>& p1, const pair<float, Vector2f>& p2) { return (p1.first < p2.first); }

float CheckLaserOverlap(const sensor_msgs::LaserScan& scan1, const Pose2Df& pose1, const sensor_msgs::LaserScan& scan2, const Pose2Df& pose2) {
  ...
  const Vector2f delta_loc = Rotation2Df(-pose1.angle) * (pose2.translation - pose1.translation);
  const float delta_angle = AngleDiff(pose2.angle, pose1.angle);
  const Affine2f delta_transform = Translation2f(delta_loc) * Rotation2Df(delta_angle);
  vector<pair<float, Vector2f> > new_point_cloud(ranges2.size());
  for (size_t i = 0; i < ranges2.size(); ++i) {
    const float scan_angle = scan2.angle_min + scan2.angle_increment * static_cast<float>(i);
    const Vector2f scan_point = delta_transform * Rotation2Df(scan_angle) * Vector2f(ranges2[i], 0.0);
    const float point_angle = atan2(scan_point.y(), scan_point.x());
    new_point_cloud[i] = std::make_pair(point_angle, scan_point);
  }
  std::sort(new_point_cloud.begin(), new_point_cloud.end(), LaserPointComparator);
  float diff = 0.0;
  size_t j = 0;
  for (size_t i = 0; i < ranges1.size(); ++i) {
    if (ranges1[i] < scan1.range_min || ranges1[i] > scan1.range_max || ranges2[i] < scan2.range_min || ranges2[i] > scan2.range_max) {
      continue;
    }
    const float scan_angle = scan1.angle_min + scan1.angle_increment * static_cast<float>(i);
    for (; j < new_point_cloud.size() && new_point_cloud[j].first < scan_angle; ++j) {
    }
...
void SRLVisualize(const vector<double>& poses, const PointCloudf& point_cloud_e, const NormalCloudf& normal_cloud, const vector<vector_localization::LTSConstraint*>& constraints) {
  ...
  for (size_t i = 0; i < num_samples; ++i) {
    ...
    pose_weights[i] = localization_->ObservationLikelihood(pose, point_cloud_e, normal_cloud);
  ...
  for (size_t i = 0; i < num_samples; ++i) {
    const Affine2f pose_transform = Translation2f(poses[3 * i + 0], poses[3 * i + 1]) * Rotation2Df(poses[3 * i + 2]);
    const Vector2f line_p0(poses[3 * i + 0], poses[3 * i + 1]);
    const Vector2f line_p1 = line_p0 + Rotation2Df(poses[3 * i + 2]) * Vector2f(0.3, 0.0);
    const double alpha = max(0.05, pose_weights[i] / max_weight);
    const uint32_t alpha_channel = (static_cast<uint32_t>(255.0 * alpha) << 24) & 0xFF000000;
  ...
  const Affine2f best_pose_transform = Translation2f(poses[3 * best_pose + 0], poses[3 * best_pose + 1]) * Rotation2Df(poses[3 * best_pose + 2]);
...
void SensorResettingResample(const sensor_msgs::LaserScan& laser_message) {
  ...
  for (size_t i = 0; i < laser_message.ranges.size(); ++i) {
    if (laser_message.ranges[i] < kMinPointCloudRange || laser_message.ranges[i] > kMaxPointCloudRange || !std::isfinite(laser_message.ranges[i])) continue;
    const float angle = laser_message.angle_min + laser_message.angle_increment * static_cast<float>(i);
    point_cloud.push_back(sensor_offset + Rotation2Df(angle) * Vector2f(laser_message.ranges[i], 0.0));
  }
  NormalCloudf normal_cloud;
  GenerateNormals(kMaxNormalPointDistance, &point_cloud, &normal_cloud);
  ...
  printf("Running SRL...");
  fflush(stdout);
  localization_->SensorResettingResample(point_cloud, normal_cloud, kNumSamples, kRadialStdDev, kTangentialStdDev, kAngularStdDev, &SRLVisualize, &poses, &pose_weights, &constraints);
  printf(" Done.\n");
  fflush(stdout);
...
void PlayBagFile(const string& bag_file, int max_poses, bool use_point_constraints, double time_skip, ros::NodeHandle* node_handle, double* observation_error_ptr, char* keyframes_file) {
  ...
  localization_options_.CorrespondenceCallback = ((debug_level_ > 0) ? CorrespondenceCallback : NULL);
  localization_->SetOptions(localization_options_);
  localization_->Initialize(Pose2Df(kStartingAngle, kStartingLocation), kMapName);
  ...
  nonblock(true);
  for (rosbag::View::iterator it = view.begin(); run_ && it != view.end(); ++it) {
    const rosbag::MessageInstance& message = *it;
    bag_time = message.getTime().toSec();
    if (bag_time_start < 0.0) {
      // Initialize bag starting time.
    ...
    if (!quiet_) {
      ...
      if (false) {
        printf("\r%02d:%02d:%04.1f (%.1f) Lost:%.3f", hh, mm, ss, elapsed_time, localization_->GetLostMetric());
      }
      printf("\r%02d:%02d:%04.1f (%.1f) Pose:%8.3f,%8.3f,%6.2f\u00b0", hh, mm, ss, elapsed_time, last_laser_pose.translation.x(), last_laser_pose.translation.y(), RadToDeg(last_laser_pose.angle));
    }
    fflush(stdout);
    int keycode = -1;
    if (run_ && kbhit() != 0) {
...

    {
      ...
      float init_angle;
      string init_map;
      if (LoadSetLocationMessage(message, &init_location, &init_angle, &init_map)) {
        if (debug_level_ > 0) {
          printf("Initializing location to %s: %f,%f, %f\u00b0\n", init_map.c_str(), init_location.x(), init_location.y(), DegToRad(init_angle));
        }
        const Pose2Df init_pose(init_angle, init_location.x(), init_location.y());
  ...
  if (statistical_test_index_ >= 0) {
    const string file_name = StringPrintf("results/enml/non_markov_test_%d.txt", statistical_test_index_);
    ScopedFile fid(file_name, "w");
    CHECK_NOTNULL(fid());
    for (unsigned int i = 0; i < pose_trajectory.size(); ++i) {
      fprintf(fid, "%f,%f,%f\n", pose_trajectory[i].translation.x(), pose_trajectory[i].translation.y(), pose_trajectory[i].angle);
    }
  }
...
void InitializeCallback(const amrl_msgs::Localization2DMsg& msg) {
  ...
  localization_->Initialize(Pose2Df(msg.pose.theta, Vector2f(msg.pose.x, msg.pose.y)), msg.map);
  localization_publisher_amrl_.publish(msg);
  localization_publisher_ros_.publish(ConvertAMRLmsgToROSmsg(msg));

  if (false) {
    const string map_file = StringPrintf("%s/%s/%s.vectormap.txt", maps_dir_.c_str(), msg.map.c_str(), msg.map.c_str());
...
void OnlineLocalize(bool use_point_constraints, ros::NodeHandle* node) {
  ...
  Subscriber laser_subscriber = node->subscribe(CONFIG_scan_topic, 1, LaserCallback);
  Subscriber odom_subscriber = node->subscribe(CONFIG_odom_topic, 1, OdometryCallback);
  Subscriber initialize_subscriber = node->subscribe("/set_pose", 1, InitializeCallback);
...
void PrintBackTrace(FILE* file) {
  ...
  for (i = 1; i < trace_size; ++i) {
    fprintf(file, "#%d %s\n", i - 1, messages[i]);
    std::string addr2line_command = StringPrintf("addr2line %p -e /proc/%d/exe", trace[i], getpid());
...
int main(int argc, char** argv) {
  ...
  static struct poptOption options[] = {{"config_dir", 'c', POPT_ARG_STRING, &config_dir_, 1, "Config directory", "STRING"},
                                        ...
                                        {"bag-file", 'b', POPT_ARG_STRING, &bag_file, 1, "ROS bagfile to use", "STRING"},
                                        {"max-poses", 'n', POPT_ARG_INT, &max_laser_poses, 1, "Maximum number of laser poses to optimize", "NUM"},
                                        {"time-skip", 's', POPT_ARG_DOUBLE, &time_skip, 1, "Time to skip from the bag file", "NUM"},
                                        {"test-set", 't', POPT_ARG_INT, &test_set_index_, 1, "Test set index", "NUM"},
                                        {"statistical-test", 'T', POPT_ARG_INT, &statistical_test_index_, 1, "Statistical test index", "NUM"},
                                        {"noise", 'N', POPT_ARG_DOUBLE, &odometry_additive_noise_, 1, "Statistical test additive random noise", "NUM"},
                                        {"keyframes", 'k', POPT_ARG_STRING, &keyframes_file, 1, "Keyframes file", "STRING"},
                                        {"episode-images", 'e', POPT_ARG_STRING, &episode_images_path, 1, "Epidode images path", "STRING"},
                                        {"unique_node_name", 'u', POPT_ARG_NONE, &unique_node_name, 0, "Use unique ROS node name", "NONE"},
                                        {"initial_poses", 'i', POPT_ARG_NONE, &return_initial_poses, 0, "Return intial, instead of final, pose estimates", "NONE"},
                                        {"disable-stfs", 'p', POPT_ARG_NONE, &disable_stfs, 0, "Disable STFs", "NONE"},
                                        {"save-ltfs", 'l', POPT_ARG_NONE, &save_ltfs_, 0, "Save LTFs", "NONE"},
                                        {"quiet", 'q', POPT_ARG_NONE, &quiet_, 0, "Quiet", "NONE"},
  ...
  const bool running_tests = (test_set_index_ >= 0 || statistical_test_index_ >= 0);
  if (running_tests) {
    const unsigned long seed = time(NULL);
    rand_ = util_random::Random(seed);
    printf("Seeding with %lu test=%.5f\n", seed, rand_.Gaussian(0, 1));
  }
  const string node_name = (running_tests || unique_node_name) ? StringPrintf("NonMarkovLocalization_%lu", static_cast<uint64_t>(GetWallTime() * 1000000.0)) : string("NonMarkovLocalization");
  ...
  if (bag_file != NULL) {
    PlayBagFile(bag_file, max_laser_poses, !disable_stfs, time_skip, &ros_node, NULL, keyframes_file);
  } else {
